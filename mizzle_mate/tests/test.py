...
class TestMizzleMateAPI:
    ...
    def test_chat_endpoint_single_module(self, monkeypatch):
        # Mock intent_classifier to return a single module
        monkeypatch.setattr(intent_classifier, "classify", lambda msg: ["ci_cd"])

        # Mock router to return a fixed response
        monkeypatch.setattr(router, "route_request", lambda mod, uid, msg: {
            "module": mod, "reply": "Test reply", "status": "success"
        })
    ...
    def test_chat_endpoint_multiple_modules(self, monkeypatch):
        # Mock intent_classifier to return multiple modules
        monkeypatch.setattr(intent_classifier, "classify", lambda msg: ["ci_cd", "logs"])

        # Mock router to return a fixed response
        monkeypatch.setattr(router, "route_request", lambda mod, uid, msg: {
            "module": mod, "reply": f"Reply for {mod}", "status": "success"
        })
